  ...
  return (
    <div className="py-12 max-w-6xl mx-auto px-6">
      <div className="mb-12 border-b border-certo-gold/30 pb-6">
        <h2 className="text-3xl font-serif font-bold text-certo-fg tracking-tight">Pipeline Status</h2>
        <p className="mt-2 text-md text-certo-fg-muted max-w-2xl font-medium tracking-wide">
      ...
      <section className="mb-12">
        <h3 className="text-2xl font-serif font-bold text-certo-fg mb-6">Aktywne zadania</h3>
        {(!runs || runs.length === 0) ? (
          <p className="text-sm text-certo-fg-muted italic p-6 border border-dashed border-certo-card-border text-center">
          ...
          <div className="grid gap-4">
            {runs?.map((run: any) => (
              <div key={run.id} className="bg-certo-card border border-certo-card-border p-5 hover:border-certo-gold hover:shadow-md transition-all duration-300 rounded-[2px] group">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <span className="font-serif font-semibold text-lg text-certo-fg group-hover:text-certo-gold transition-colors">Issue #{run.issue_number}</span>
      ...
      {/* Recent reviews */}
      <section>
        <h3 className="text-2xl font-serif font-bold text-certo-fg mb-6">Ostatnie recenzje <span className="text-certo-gold">(Gemini)</span></h3>
        {(!reviews || reviews.length === 0) ? (
          ...
          <div className="grid gap-4">
            {reviews?.map((review: any) => (
              <div key={review.id} className="bg-certo-card border border-certo-card-border p-5 hover:border-certo-gold hover:shadow-md transition-all duration-300 rounded-[2px] group">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <span className="font-serif font-semibold text-lg text-certo-fg group-hover:text-certo-gold transition-colors">{review.documents?.title || "—"}</span>
