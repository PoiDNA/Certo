  ...
  return (
    <div className="w-full">
      ...
      <section className="max-w-4xl mx-auto px-6 py-16">
        <h2 className="text-2xl font-serif font-bold text-certo-fg mb-8">{t('profiles_title')}</h2>
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {profiles.map((profile) => (
            <div key={profile} className="bg-certo-card p-6 border border-certo-card-border hover:border-certo-gold/30 transition-colors">
              <h3 className="font-serif font-extrabold text-certo-fg text-[2.625rem] leading-tight md:text-xl mb-2">{t(`profile_${profile}`)}</h3>
              <p className="text-sm text-certo-fg-muted leading-relaxed">{t(`profile_${profile}_desc`)}</p>
            </div>
          ))}
        </div>

        {/* Accreditation */}
        <div className="bg-certo-surface border-l-4 border-certo-gold p-8 mb-16">
          <h2 className="text-2xl font-serif font-bold text-certo-fg mb-3">{t('accreditation_title')}</h2>
          <p className="text-sm text-certo-fg-muted leading-relaxed">{t('accreditation_desc')}</p>
        </div>
