  ...
  return (
    <div className="py-12 max-w-6xl mx-auto px-6">
      <div className="mb-12 border-b border-certo-gold/30 pb-6">
        <h2 className="text-3xl font-serif font-bold text-certo-fg tracking-tight">Dokumentacja Sprint 0</h2>
        <p className="mt-2 text-md text-certo-fg-muted max-w-2xl">
...
function DocumentSection({ locale, title, subtitle, docs }: { 
  ...
  return (
    <div className="mb-12">
      <h3 className="text-2xl font-serif font-bold text-certo-fg mb-1">{title}</h3>
      <p className="text-sm text-certo-fg-muted mb-6 font-medium tracking-wide uppercase">{subtitle}</p>
      ...
      <div className="grid gap-4">
        {docs.map(doc => (
          <Link 
            ...
            className="block bg-certo-card border border-certo-card-border p-5 hover:border-certo-gold hover:shadow-md transition-all duration-300 rounded-[2px] group"
          >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div>
                <h4 className="font-serif font-semibold text-lg text-certo-fg group-hover:text-certo-gold transition-colors">{doc.title}</h4>
                <p className="text-xs text-certo-fg-muted mt-2 font-medium tracking-wide uppercase">
