  ...
  if (!authenticated) {
    return (
      <div className="max-w-md mx-auto mt-20">
        <h2 className="font-serif font-bold text-certo-fg text-xl mb-4 text-center">Autoryzacja</h2>
        <div className="space-y-4">
          <input
            ...
            className="w-full px-4 py-3 border border-certo-card-border rounded-xl text-sm bg-certo-surface text-certo-fg"
          />
          <button
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-certo-card rounded-xl p-4 border border-certo-card-border text-center">
          <div className="text-2xl font-bold text-certo-fg">{stats.total}</div>
          <div className="text-xs text-certo-fg-muted">Wszystkie</div>
        </div>
        ...
        {['all', 'new', 'reviewed', 'accepted', 'rejected'].map((s) => (
          <button
            ...
            className={`px-4 py-2 text-xs rounded-lg transition-colors ${
              filter === s ? 'bg-certo-navy text-white' : 'bg-certo-card border border-certo-card-border text-certo-fg-muted hover:border-certo-navy/30'
            }`}
          >
      ...
      <div className="space-y-3">
        {apps.length === 0 ? (
          <div className="bg-certo-card rounded-xl p-8 text-center text-certo-fg-muted">Brak zgłoszeń</div>
        ) : (
          apps.map((app) => (
            <div key={app.id} className="bg-certo-card rounded-xl border border-certo-card-border overflow-hidden">
              {/* Header row */}
              <div
                className="flex items-center gap-4 p-4 cursor-pointer hover:bg-certo-cream/30 hover:bg-certo-surface transition-colors"
              ...
              >
                ...
                <span className="font-medium text-sm text-certo-fg flex-1">{app.organization_name}</span>
              ...
              {/* Expanded details */}
              {expanded === app.id && (
                <div className="border-t border-certo-card-border p-4 bg-certo-cream/20 bg-certo-surface space-y-4">
