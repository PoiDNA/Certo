...
const CHEVRON = (
  <svg aria-hidden="true" className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-certo-navy/30 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    return (
      <div className="bg-white rounded-2xl shadow-xl p-10 text-center max-w-xl mx-auto">
        <div className="w-20 h-20 bg-certo-gold/10 rounded-full flex items-center justify-center mx-auto mb-8">
          <svg aria-hidden="true" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#CC9B30" strokeWidth="2">
      ...
      <div className="p-8 pt-4">
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <label className="sr-only" htmlFor="country">Kraj</label>
                <select id="country" value={fd.country} onChange={(e) => { set('country', e.target.value); setLookupState('idle'); }} className={`${input} appearance-none pr-10 cursor-pointer`}>
              ...
              <div className="bg-certo-cream/40 rounded-xl p-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <input value={fd.nip} onChange={(e) => { set('nip', e.target.value); setLookupState('idle'); }} placeholder={fd.country === 'PL' ? 'NIP (np. 5252344078)' : `VAT ID (np. ${fd.country}123456789)`} aria-label="NIP lub VAT ID" className={input} />
                  {fd.country === 'PL' && (
                    <input value={fd.krs} onChange={(e) => { set('krs', e.target.value); setLookupState('idle'); }} placeholder="KRS (opcjonalnie)" aria-label="KRS" className={input} />
                  )}
                </div>
                <button
                  type="button"
              ...
              <input id="org-name" value={fd.organization_name} onChange={(e) => { set('organization_name', e.target.value); setStepError(false); }} placeholder={`${fd.applicant_type === 'observer' ? t('form_org_name_observer') : t('form_org_name')} *`} aria-label="Nazwa podmiotu" aria-required="true" {...(stepError && !fd.organization_name ? { 'aria-describedby': 'org-error' } : {})} className={`${input} ${lookupState === 'found' && fd.organization_name ? 'border-green-300 bg-green-50/30' : ''} ${stepError && !fd.organization_name ? 'border-red-300 bg-red-50/30' : ''}`} />

              <div className="relative">
                <select id="sector" value={fd.sector} onChange={(e) => { set('sector', e.target.value); setStepError(false); }} aria-label="Sektor" aria-required="true" className={`${input} appearance-none pr-10 cursor-pointer ${stepError && !fd.sector ? 'border-red-300 bg-red-50/30' : ''}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <input value={fd.city} onChange={(e) => set('city', e.target.value)} placeholder={t('form_city')} aria-label="Miasto" className={`${input} ${lookupState === 'found' && fd.city ? 'border-green-300 bg-green-50/30' : ''}`} />
                <input value={fd.postal_code} onChange={(e) => set('postal_code', e.target.value)} placeholder={t('form_postal_code')} aria-label="Kod pocztowy" className={`${input} ${lookupState === 'found' && fd.postal_code ? 'border-green-300 bg-green-50/30' : ''}`} />
              </div>

              <input value={fd.address} onChange={(e) => set('address', e.target.value)} placeholder={t('form_address')} aria-label="Adres" className={`${input} ${lookupState === 'found' && fd.address ? 'border-green-300 bg-green-50/30' : ''}`} />

              {/* Website */}
              <input value={fd.website} onChange={(e) => set('website', e.target.value)} type="url" placeholder={t('form_website')} aria-label="Strona internetowa" className={input} />
            </div>
            {stepError && (
              <p id="org-error" role="alert" className="text-xs text-red-500 mt-2 text-center">Wypełnij wymagane pola oznaczone *</p>
            )}
        ...
        {step === 'contact' && (
          <div>
            ...
            <div className="space-y-4">
              <input value={fd.contact_person} onChange={(e) => { set('contact_person', e.target.value); setStepError(false); }} placeholder={`${t('form_contact_person')} *`} aria-label="Imię i nazwisko" aria-required="true" className={`${input} ${stepError && !fd.contact_person ? 'border-red-300 bg-red-50/30' : ''}`} />
              {fd.applicant_type === 'representative' && (
                <input value={fd.role} onChange={(e) => set('role', e.target.value)} placeholder={t('form_role')} aria-label="Stanowisko" className={input} />
              )}
              <input value={fd.email} onChange={(e) => { set('email', e.target.value); setStepError(false); }} type="email" placeholder={`${t('form_email')} *`} aria-label="Email" aria-required="true" className={`${input} ${stepError && !fd.email ? 'border-red-300 bg-red-50/30' : ''}`} />
              <input value={fd.phone} onChange={(e) => set('phone', e.target.value)} type="tel" placeholder={t('form_phone')} aria-label="Telefon" className={input} />
              {fd.applicant_type === 'observer' && (
                <input value={fd.relation} onChange={(e) => set('relation', e.target.value)} placeholder={t('form_relation_placeholder')} aria-label="Relacja z podmiotem" className={input} />
              )}
            </div>
            {stepError && (
              <p role="alert" className="text-xs text-red-500 mt-2 text-center">Wypełnij wymagane pola oznaczone *</p>
            )}
            <button type="button" onClick={goNext} className="w-full mt-4 bg-certo-navy text-certo-gold py-3.5 rounded-xl text-sm font-semibold hover:bg-certo-gold hover:text-white transition-colors duration-300">
              Dalej →
            </button>
          ...
          <div>
            ...
            <div className="space-y-4">
              <textarea
                ...
                aria-label="Motywacja zgłoszenia"
                aria-required="true"
              ...
              {state === 'error' && (
                <div role="alert" className="text-sm text-red-600 text-center space-y-1">
                  <p>{t('form_error')}</p>
