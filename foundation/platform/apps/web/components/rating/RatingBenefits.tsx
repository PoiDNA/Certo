...
export default function RatingBenefits() {
  ...
  return (
    ...
    >
      <h2 className="text-3xl md:text-4xl font-serif font-bold text-certo-fg mb-10 text-center">
        {t('benefits_title')}
      </h2>
        ...
        {tabs.map((tab, i) => (
          <button
            ...
            className={`flex-1 flex items-center gap-3 p-4 sm:p-5 rounded-xl border-2 transition-all duration-300 text-left ${
              i === active
                ? 'border-certo-gold bg-certo-card shadow-lg shadow-certo-gold/10'
                : 'border-transparent bg-certo-surface hover:bg-certo-card hover:border-certo-navy/10'
            }`}
          >
            ...
            <span className={`font-serif font-bold text-sm sm:text-base transition-colors duration-300 ${
              i === active ? 'text-certo-fg' : 'text-certo-fg-muted'
            }`}>
      ...
      <div className={`${current.lightBg} rounded-xl p-8 md:p-10 animate-fadeIn`} key={active}>
        <p className="text-base md:text-lg text-certo-fg-muted leading-relaxed mb-8 max-w-3xl">
          ...
          {Array.from({ length: current.items }, (_, i) => (
            <div key={i} className="flex items-start gap-3">
              <div className={`w-6 h-6 rounded-full ${tabs[active].accent} flex items-center justify-center shrink-0 mt-0.5`}>
                <svg aria-hidden="true" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              </div>
              <span className="text-sm md:text-base text-certo-fg-muted leading-relaxed">
