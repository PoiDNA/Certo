  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className={`flex items-center gap-1.5 text-certo-teal-darker hover:text-certo-teal transition-colors duration-200 text-sm font-medium py-2 md:py-0 ${isPending ? 'opacity-50' : ''}`}
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-certo-teal/20 rounded shadow-xl z-50 overflow-hidden">
          <div className="p-2">
            <input
              ref={inputRef}
              type="text"
              ...
              placeholder="Search..."
              className="w-full px-2 py-2 text-base md:text-xs bg-transparent border border-certo-teal/20 rounded text-certo-teal-darker placeholder:text-certo-teal/40 focus:outline-none focus:border-certo-teal/60"
            />
          </div>
          <div className="max-h-80 md:max-h-64 overflow-y-auto px-1 pb-2">
            {filteredGroups.map((group) => (
              <div key={group.label}>
                <div className="px-2 pt-2 pb-1 text-[10px] uppercase tracking-wider text-certo-teal/50 font-medium">
                  {group.label}
                </div>
                  ...
                  <button
                    ...
                    className={`w-full text-left px-2 py-2.5 md:py-1 text-sm md:text-xs rounded transition-colors duration-150 flex items-center gap-2 ${
                      locale === currentLocale
                        ...
                        : 'text-certo-teal-darker hover:text-certo-teal hover:bg-certo-gray-light'
                    }`}
                  >
                    <span className="w-5 text-certo-teal/40 font-mono">{locale.toUpperCase()}</span>
