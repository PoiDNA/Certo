export default function Home() {
  return (
    <div className="flex flex-col min-h-screen bg-certo-gray-light">
      ...
      {/* Stats Section */}
      <section className="bg-white border-b border-certo-gray py-12 px-6">
        <div className="mx-auto max-w-6xl grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            { label: 'Standardy ISO', value: '27001 · 37301' },
            { label: 'Zgodność regulacyjna', value: 'EU · PL' },
            { label: 'Weryfikacja tożsamości', value: 'KYC / AML' },
            { label: 'Ocena governance', value: 'Certo Vector' },
          ].map((stat, i) => (
            <div key={i} className="flex flex-col p-4 border-l-2 border-certo-teal/20">
              <span className="text-xs font-mono font-bold text-certo-gray-dark uppercase tracking-widest mb-2">{stat.label}</span>
              <span className="text-xl md:text-2xl font-bold text-certo-teal-darker">{stat.value}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Services Section */}
      <section id="uslugi" className="py-24 px-6 mx-auto max-w-6xl w-full">
        <div className="mb-16">
          <h2 className="text-xs font-mono font-bold text-certo-teal uppercase tracking-widest mb-3">USŁUGI</h2>
          <h3 className="text-3xl md:text-4xl font-bold text-certo-teal-darker tracking-tight max-w-2xl">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <div key={s.n} className="bg-white border border-certo-gray p-8 hover:border-certo-teal/50 hover:shadow-lg transition-all duration-300 rounded-[2px] group relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-6">
                <span className="text-sm font-mono font-bold text-certo-gray-dark">{s.n}</span>
                <span className="text-[10px] font-bold tracking-widest uppercase px-2 py-1 bg-certo-gray-light text-certo-teal-dark rounded-sm">{s.tag}</span>
              </div>
              <h4 className="text-xl font-bold text-certo-teal-darker mb-4 group-hover:text-certo-teal transition-colors">{s.t}</h4>
              <p className="text-sm text-certo-gray-dark leading-relaxed font-medium">{s.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="technologia" className="py-24 px-6 bg-white border-t border-certo-gray">
        <div className="mx-auto max-w-6xl">
          <div className="mb-16 text-center">
            <h2 className="text-xs font-mono font-bold text-certo-teal uppercase tracking-widest mb-3">ARCHITEKTURA</h2>
            <h3 className="text-3xl md:text-4xl font-bold text-certo-teal-darker tracking-tight">
              Jak działa Certo ID
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.n} className="relative p-6 bg-certo-gray-light border border-certo-gray rounded-[2px] z-10">
                ...
                <h4 className="text-lg font-bold text-certo-teal-darker mb-2">{s.t}</h4>
                <p className="text-xs text-certo-gray-dark leading-relaxed font-medium">{s.d}</p>
              </div>
            ))}
          </div>
        </div>
